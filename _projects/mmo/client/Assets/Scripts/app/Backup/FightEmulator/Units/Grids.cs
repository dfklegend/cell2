...
{	
    ...
    public class Grids
    {
        ...
        {
            // 找合适的第一个位置
            float gridCenterX = (float)Math.Floor(centerX / GridSize) * GridSize - centerX;
            float gridCenterY = (float)Math.Floor(centerY / GridSize) * GridSize - centerY;
            float startX = gridCenterX - (GridsWidth / 2) * GridSize;
            float startY = gridCenterY - (GridsHeight / 2) * GridSize;
            ...
            {
                x = startX;
                for(var col = 0; col < GridsWidth; col ++, x += GridSize, index ++)
                {
