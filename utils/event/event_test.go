...
// 测试并发安全性
// 并发订阅/取消订阅，publish
func dotestConcurrence(t *testing.T, fakeMutex bool) {
	gotPanic := false
...
func Test_ConcurrenceNormal(t *testing.T) {
	...
	dotestConcurrence(t, false)
}

// concurrent map iteration and map write 不是panic
func NoTest_ConcurrenceFakeMutex(t *testing.T) {
	log.Printf("---- Test_ConcurrenceFakeMutex ----")
	log.Printf("---- 应该有map竞争冲突 ----")

	assert.Panics(t, func() {
		dotestConcurrence(t, true)
	})
}
