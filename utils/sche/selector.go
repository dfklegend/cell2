...
type MultiSelector struct {
	...
	//
	LastSelectCost    int64
	LastDoCost        int64
	LastDoChannelName string
	//
...
func NewMultiSelector() *MultiSelector {
	m := &MultiSelector{
		...
		dirty:     false,
		chanDirt:  make(chan int, 10),
	}

	// 当有变化时，send消息，唤醒处理，更即时
	m.AddSelector("__dirt__", NewFuncSelector(reflect.ValueOf(m.chanDirt), func(v reflect.Value, recvOk bool) {
		//
	}))
...
func (s *MultiSelector) AddSelector(name string, selector IChanSelector) {
...
func (s *MultiSelector) HandleOnce() {
	...
	beginTime := common.NowNano()
	chosen, recv, recvOk := reflect.Select(cases)
	s.LastSelectCost = common.NowNano() - beginTime
	...
	s.LastDoChannelName = data.name
	beginTime = common.NowNano()
	data.selector.DoTask(s, recv, recvOk)
	s.LastDoCost = common.NowNano() - beginTime
