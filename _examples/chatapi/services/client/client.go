...
func (s *Service) loginToChat(id int32, chatServiceID string, roomID int32, token int32) {
	...
	s.RequestEx(pid, "chat.join", &mymsg.ReqJoin{
	...
	//s.RequestEx(pid, "sys.hello", &mymsg.ReqJoin{
	//	Sender: s.Context.Self(),
	//	ID:     id,
	//	RoomID: roomID,
	//	Token:  token,
	//	Name:   s.name,
	//}, func(err error, rawMsg any) {
	//	log.Println("sys.hello ret")
	//})
}
...
func (s *Service) say(str string) {
	...
	//	Str: str,
	//}, nil)
	s.RequestEx(s.pidChatService, "chat.say", &mymsg.ReqChat{
		ID:  s.id,
		Str: str,
	}, nil)
}
...
func (g *Creator) Create(name string) {
	...
	props, _ := as.NewServicePropsWithNewScheDisp(func() actor.Actor { return NewService() },
		name)
